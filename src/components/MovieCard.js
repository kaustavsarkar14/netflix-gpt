    ...
    >
      ...
      {isHovering && (
        <div className="absolute flex gap-4 flex-col justify-between items-start bottom-0 p-1 md:p-5  w-full">
          <h1 className="md:text-xl text-sm font-semibold">{movie.original_title}</h1>
          ...
          <div className="absolute right-5 bottom-5 hidden md:block" ><PlayCircleOutlineRoundedIcon sx={{scale:"1.4",marginBottom:"5px"}}/></div>
        </div>
      )}
    </div>
  );
